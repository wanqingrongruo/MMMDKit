...
详细的集成步骤、配置方法和高阶自定义（如修改字体颜色、监听点击和代码高亮等），请查阅 [MMMDKit 使用与配置教程](Docs/UsageTutorial.md)。
...
SPM 是当前推荐的 demo 与完整功能接入方式。通过 SPM 引入 `MMMDUIKit` / `MMMDAppKit` 时，会传递引入 SwiftMath，`$$...$$` block math 默认使用原生公式排版。
